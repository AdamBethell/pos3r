...
from pos3r.datasets.nocs import NOCS
...
def main():
    dataset = NOCS(ROOT='../RGBGenPose/data', resolution=224, source='Real', mode='test', all_objects=True)
    pretrained = "checkpoints/pos3r_nocs/checkpoint-10.pth"

    device = "cuda" if torch.cuda.is_available() else "cpu"
    device = torch.device(device)

    batch = dataset.__getitem__(4)
