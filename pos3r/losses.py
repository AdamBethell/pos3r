...
from pos3r.inference import get_pred_pts3d, find_opt_scaling
from pos3r.utils.geometry import inv, geotrf, normalize_pointcloud
from pos3r.utils.geometry import get_joint_pointcloud_depth, get_joint_pointcloud_center_scale
...
class Pos3rCriterion(nn.Module):
    def __init__(self):
        super().__init__()
        self.ray_metric = L2Loss()
        self.pt_metric = L21Loss()
    def forward(self, gt1, gt2, pred1, pred2, mask,):
        l1 = self.pt_metric.distance(pred1['pts3d'][mask], gt1[mask])
        l2 = self.ray_metric.distance(pred2['pts3d'][mask], gt2[mask])
        # print("L2")
        # print(l2)
        self_name = type(self).__name__
        details = {self_name + '_pts3d_1': float(l1.item()), self_name + '_pts3d_2': float(l2.item())}
        return l1 + l2, (details | {})
...
class LLoss (BaseCriterion):
    ...
    def forward(self, a, b):
        ...
        # assert a.shape == b.shape and a.ndim >= 2 and 1 <= a.shape[-1] <= 6, f'Bad shape = {a.shape}'
...
class L21Loss (LLoss):
    ...
    def distance(self, a, b):
        return torch.mean(torch.norm(a - b, dim=-1))  # normalized L2 distance
...
class L2Loss (LLoss):
    ...
    def distance(self, a, b):
        return torch.mean((a - b) ** 2)  # normalized L2 distance
...
class Regr3D (Criterion, MultiLoss):
    ...
    def __init__(self, criterions, norm_mode='avg_dis', gt_scale=False):
        print(criterions)
        if isinstance(criterions, tuple):
            criterion1, criterion2 = criterions
            multiple_crits = True
        else:
            criterion1 = criterions
            multiple_crits = False
        super().__init__(criterion1)
        if multiple_crits:
            self.multiple_crits = True
            self.criterion2 = Criterion(criterion2)
        else:
            self.multiple_crits = False
    ...
    def compute_loss(self, gt1, gt2, pred1, pred2, mask, **kw):
        ...
        # print(gt1.shape)
        # print(mask.shape)
        # print(pred1['pts3d'][mask].shape)
        # print(gt1[mask].shape)
        l1 = self.criterion(pred1['pts3d'][mask], gt1[mask])
        # print("L1")
        # print(l1)
        # loss on gt2 side
        if self.multiple_crits: 
            l2 = self.criterion2.criterion(pred2['pts3d'][mask], gt2[mask])
        else:
            l2 = self.criterion(pred2['pts3d'][mask], gt2[mask])
        # print("L2")
        # print(l2)
        self_name = type(self).__name__
        details = {self_name + '_pts3d_1': float(l1.mean()), self_name + '_pts3d_2': float(l2.mean())}
        return Sum((l1, mask), (l2, mask)), (details | {})
...
class ConfLoss (MultiLoss):
    ...
    def compute_loss(self, gt1, gt2, pred1, pred2, mask, **kw):
        ...
        ((loss1, msk1), (loss2, msk2)), details = self.pixel_loss(gt1, gt2, pred1, pred2, mask, **kw)
        if loss1.numel() == 0:
